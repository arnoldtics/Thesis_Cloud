...
titles_embeddings = pickle.load(open("../DeepLearningModel2/SearchEngine/titles_embeddings", "rb")) 
...
def presearch(query:str):
    ...
    k = 100 # Adjust value of k results of the presearch
...
def prediction():
    ...
    thesis_titles = search(text, presearch(text))
    ...
    for title in thesis_titles:
        # Many results with the same title...
        ...
        for _, row in matching_rows.iterrows():
            ...
            if not instance in results:
                results.append(instance)
    
    return render_template("prediction.html", query=text, results=results)
...
# Any undefined path returns to the main
@app.route('/<path:path>')
def catch_all(path):
    return render_template('index.html')

if __name__ == '__main__':
    # Debug = True just in testning. NOT PRODUCTION
    # It allows see automatic changes
